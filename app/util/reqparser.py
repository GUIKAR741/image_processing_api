...
from flask import abort as flask_abort, jsonify
import decimal
import six
...
class Argument(object):
    ...
    def __repr__(self):
        ...
        return "{0}('{1}', default={2}, dest={3}, required={4}, ignore={5}, location={6}, " \
               ...
               "operators={12}, store_missing={13}, trim={14}, nullable={15})".format(
                   self.__class__.__name__, self.name, self.default, self.dest, self.required,
                   self.ignore, self.location, self.type, self.choices, self.action,
                   self.help, self.case_sensitive, self.operators, self.store_missing,
                   self.trim, self.nullable)
    ...
    def source(self, request):
        ...
        if isinstance(self.location, six.string_types):
            ...
            values = MultiDict()
            for locate in self.location:
                value = getattr(request, locate, None)
                if callable(value):
                    value = value()
                if value is not None:
                    values.update(value)
...
class RequestParser(object):
    ...
    def copy(self):
        """Create a copy of this RequestParser with the same set of arguments."""
